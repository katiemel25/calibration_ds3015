/*
 ...
 NOTE: peak_residuals() needs "Time" as the input, referring to the number of seconds the simulation ran. As the simulation was written to demonstrate events over 10 days, the necessary function input in this case is 864000.
...
void peak_residuals(double Time)
{
  ...
  t2615->AddText("Residuals by Peak");
  ...
  for (Int_t i = 1; i <= nbins; i++) {
	...
	Double_t diff = (Peak2615sim->GetBinContent(i) - Peak2615->GetBinContent(i)) / Peak2615->GetBinContent(i);
      ...
      {
	//leaving blank to eliminate channel values = 0
      }
 ...
 for (Int_t i = 1; i <= nbins; i++) {
  ...
  residuals_2615->GetXaxis()->SetTitleSize(14);
  residuals_2615->GetXaxis()->SetTitleOffset(1);
  residuals_2615->GetYaxis()->SetTitle("(Data-Simulation)/Data");
  residuals_2615->GetYaxis()->SetTitleFont(63);
  residuals_2615->GetYaxis()->SetTitleSize(14);
  residuals_2615->GetYaxis()->SetTitleOffset(1);
  residuals_2615->GetYaxis()->SetLabelFont(63);
  residuals_2615->GetYaxis()->SetLabelSize(14);
  residuals_2615->GetYaxis()->SetRangeUser(-2, 2);

  for (Int_t i = 1; i <= nbins; i++) {
    if (Peak2615->GetBinContent(i) != 0)
      {
	Double_t diff = (Peak2615->GetBinContent(i) - Peak2615sim->GetBinContent(i)) / Peak2615->GetBinContent(i);
      }
    else 
      {
  ...
  residuals_2615->SetMarkerColor(kRed-7);
  residuals_2615->Draw("P COLZ");

 for (Int_t i = 1; i <= nbins; i++) {
    if (Peak969->GetBinContent(i) != 0)
      {
	Double_t diff = (Peak969->GetBinContent(i) - Peak969sim->GetBinContent(i)) / Peak969->GetBinContent(i);
      }
    else 
      {
	Double_t diff = Peak969->GetBinContent(i);
  ...
  for (Int_t i = 1; i <= nbins; i++) {
    if (Peak911->GetBinContent(i) != 0)
      {
	Double_t diff = (Peak911->GetBinContent(i) - Peak911sim->GetBinContent(i)) / Peak911->GetBinContent(i);
      }
    else 
      {
	Double_t diff = Peak911->GetBinContent(i);
  ...
  for (Int_t i = 1; i <= nbins; i++) {
    if (Peak583->GetBinContent(i) != 0)
      {
	Double_t diff = (Peak583->GetBinContent(i) - Peak583sim->GetBinContent(i)) / Peak583->GetBinContent(i);
      }
    else 
      {
	Double_t diff = Peak583->GetBinContent(i);
  ...
  for (Int_t i = 1; i <= nbins; i++) {
    if (Peak338->GetBinContent(i) != 0)
      {
	Double_t diff = (Peak338->GetBinContent(i) - Peak338sim->GetBinContent(i)) / Peak338->GetBinContent(i);
      }
    else 
      {
	Double_t diff = Peak338->GetBinContent(i);
  ...
  for (Int_t i = 1; i <= nbins; i++) {
    if (Peak239->GetBinContent(i) != 0)
      {
	Double_t diff = (Peak239->GetBinContent(i) - Peak239sim->GetBinContent(i)) / Peak239->GetBinContent(i);
      }
    else 
      {
        Double_t diff = Peak239->GetBinContent(i);
  ...
  c4->SaveAs("../graphs/ds3018_updated/all_residuals_w/zeros.pdf");
  */
